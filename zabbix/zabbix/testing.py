    ...
    def getCurIssue(self, zapi):
        ...
        for t in triggers:
            if int(t['value']) == 1:
                ...
                dtime_list.append(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(float(t['lastchange']))))
                level_list.append(self.prioritytostr[t['priority']])
                host_list.append(t['hosts'][0]['host'])
                group_list.append(self.getHostgroupName(zapi, t['hosts'][0]['host']))
                desc_list.append(t['description'])
        # return triggerlist
        return dtime_list,level_list,host_list,group_list,desc_list
